...
| - First get a new session id (if you don't have one already) by running<br>`$(which codex) exec --skip-git-repo-check "/clear" 2>&1 \| grep "session id"` | - First get a new session_id (if you don't have one already) by running<br>`$(which claude) --permission-mode "bypassPermissions" --output-format json -p "/clear"` | | - First create a new session (if you didn't do that already) by running<br>`$(which copilot) --allow-all-paths --allow-all-tools -p "read @/home/vscode/.copilot/AGENTS.md and respond with 'OK'" 2> /dev/null` |
